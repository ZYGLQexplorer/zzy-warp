...
if [[ ! -f /usr/local/bin/nf ]]; then
    wget https://raw.githubusercontent.com/ZYGLQexplorer/zzy-warp/main/files/nfverify/nf_linux_$(archAffix) -O /usr/local/bin/nf
...
checktun(){
    if [[ ! $TUN =~ "in bad state"|"处于错误状态"|"ist in schlechter Verfassung" ]]; then
        if [[ $VIRT == lxc ]]; then
            ...
            wget -N --no-check-certificate https://raw.githubusercontent.com/ZYGLQexplorer/zzy-warp/main/files/tun.sh && bash tun.sh
        else
            red "检测到目前VPS未开启TUN模块, 请到后台控制面板处开启"
            exit 1
        fi
...
initwgcf(){
    wget -N --no-check-certificate https://raw.githubusercontent.com/ZYGLQexplorer/zzy-warp/main/files/wgcf/wgcf-latest-linux-$(archAffix) -O /usr/local/bin/wgcf
...
installwgcf(){
    ...
    if [[ $SYSTEM == "CentOS" ]]; then
        ...
        if [[ $OSID == 9 ]] && [[ -z $(type -P resolvconf) ]]; then
            wget -N https://raw.githubusercontent.com/ZYGLQexplorer/zzy-warp/main/files/resolvconf -O /usr/sbin/resolvconf
    ...
    if [[ $main -lt 5 ]] || [[ $minor -lt 6 ]] || [[ $VIRT =~ lxc|openvz ]]; then
        wget -N --no-check-certificate https://raw.githubusercontent.com/ZYGLQexplorer/zzy-warp/main/files/wireguard-go/wireguard-go-$(archAffix) -O /usr/bin/wireguard-go
...
installWireProxy(){
    ...
    wget -N https://raw.githubusercontent.com/ZYGLQexplorer/zzy-warp/main/files/wireproxy/wireproxy-$(archAffix) -O /usr/local/bin/wireproxy
...
warpsw1(){
    ...
    if [[ $accountInput == 3 ]]; then
        ...
        if [[ -z $teamconfigurl ]]; then
            ...
            teamconfigurl="https://raw.githubusercontent.com/ZYGLQexplorer/zzy-warp/main/files/publicteam.xml"
        fi
...
menu(){
    ...
    echo " ----------------------------------------------------------------------------------"
    echo -e " ${GREEN}11.${PLAIN} 安装 Wireproxy-WARP 代理模式                | ${GREEN}16.${PLAIN} 获取 WARP+ 账户流量"
    echo -e " ${GREEN}12.${PLAIN} 修改 Wireproxy-WARP 代理模式连接端口        | ${GREEN}17.${PLAIN} 切换 WARP 账户类型"
    echo -e " ${GREEN}13.${PLAIN} 开启或关闭 Wireproxy-WARP 代理模式          | ${GREEN}18.${PLAIN} 获取解锁 Netflix 的 WARP IP"
    echo -e " ${GREEN}14.${PLAIN} ${RED}卸载 WireProxy-WARP 代理模式${PLAIN}                | ${GREEN}0.${PLAIN} 退出脚本"
    echo -e " ${GREEN}15.${PLAIN} 提取 WireGuard 节点配置文件"
    echo -e ""
    showIP
    echo -e ""
    read -rp "请输入选项 [0-18]：" menuChoice
    case $menuChoice in
        0) exit 1 ;;
        1) wgcfv44 ;;
        2) wgcfv66 ;;
        ...
        16) warpup ;;
        17) warpsw ;;
        18) wget -N --no-check-certificate https://raw.githubusercontent.com/ZYGLQexplorer/zzy-warp/main/netflix.sh && bash netflix.sh ;;
